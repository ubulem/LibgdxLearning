...
class Love implements ApplicationListener, InputProcessor {
    ...
    private void prepareHeartsTextures(FileHandle fileHandle) {
    ...
    private float[][][] prepareHslData(FileHandle fileHandle) {
    ...
    private Texture generateHeartTexture(float[][][] hslData, float newHue) {
    ...
    private void updatePhysics() {
        ...
        for (Heart heart : hearts.values()) {
            ...
            if (Gdx.input.isKeyPressed(Keys.UP))
                heartBody.applyForceToCenter(0, 20000, true);

            if (Gdx.input.isKeyPressed(Keys.DOWN))
                heartBody.applyForceToCenter(0, -20000, true);

            if (Gdx.input.isKeyPressed(Keys.LEFT))
                heartBody.applyForceToCenter(-20000, 0, true);

            if (Gdx.input.isKeyPressed(Keys.RIGHT))
                heartBody.applyForceToCenter(20000, 0, true);

            if (Gdx.input.isKeyPressed(Keys.Q))
                heartBody.applyTorque(10000, true);
